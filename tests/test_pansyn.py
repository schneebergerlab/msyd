...
@pytest.mark.parametrize("detect_crosssyn", [False, True])
@pytest.mark.parametrize("allow_overlap", [False, True])
@pytest.mark.parametrize("cg", [False, True])
def test_cov_basic(overlapping_pansyns, cg, detect_crosssyn, allow_overlap):
    """
    ...
    if not cg:
        l.cigars_dict = None
        r.cigars_dict = None
    res = calc_overlap(l, r, detect_crosssyn=detect_crosssyn, allow_overlap=allow_overlap)
...
ov_overlapping_nocg = Pansyn(Range('test', 1, 'NaN', 151, 200),
        {
...
ov_overlapping = Pansyn(Range('test', 1, 'NaN', 151, 200),
        {
...
def test_cov_nocg(overlapping_pansyns):
    """
    ...
    l.cigars_dict = None
    r.cigars_dict = None
    res = calc_overlap(l, r)
    ov = res[0]
    assert ov == ov_overlapping_nocg

def test_cov(overlapping_pansyns):
    """
    ...
    res = calc_overlap(l, r)
    ov = res[0]
    assert ov == ov_overlapping

def test_cov_cross_nocg(overlapping_pansyns):
    """
    ...
    l.cigars_dict = None
    r.cigars_dict = None
    res = calc_overlap(l, r, detect_crosssyn=True)

    assert len(res) == 3
    assert res[1] == ov_overlapping_nocg # middle should be overlap

    #TODO proper checks
    assert len(res[0].ref) == r.ref.start - l.ref.start +1
    assert len(res[2].ref) == r.ref.end - l.ref.end +1
